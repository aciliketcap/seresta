...
class CarWowRawUsedCarListingORM(BaseRawUsedCarListingORM):
    """Joined-table ORM for CarWow job postings; FK back to ``raw_used_car_listing.id``."""
...
class CarWowRawUsedCarListingSqlAlchemyRepository(BaseRawUsedCarListingSqlAlchemyRepository):
    ...
    def add(self, used_car_listing: CarWowRawUsedCarListing) -> None:
        super().add(used_car_listing)
        with self._sm.begin() as session:
            session.merge(self.ORM(**used_car_listing.model_dump(mode="json")))

    def get(self, used_car_listing_id: str) -> CarWowRawUsedCarListing | None:
