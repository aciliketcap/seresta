...
# `serespar/pyproject.toml` file:
Contains the dependencies of the base library. Note that the dependencies are grouped here.
local-ide-needs: We install everything into our Docker image but the IDE running on the host still needs a shallow copy of the dependencies so that the internal linter doesn't underline the whole code with "xxx is not defined" errors.
